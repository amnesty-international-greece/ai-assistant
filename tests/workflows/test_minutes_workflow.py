...
async def test_step_approval_and_share_test_mode(workflow):
    """In test_mode, redirects email to test_email."""
    ...
    with patch("src.workflows.board_meeting_minutes.settings") as mock_settings:
        ...
        mock_settings.testing.test_email = "secgen@test.org"
...
async def test_step_finalize(workflow, tmp_path):
    ...
    upload_kwargs = workflow._onedrive.upload_file.call_args.kwargs
    assert upload_kwargs["remote_folder"] == "Αρχείο ανά έτος/2026"
    assert upload_kwargs["filename"] == "[2026_006] Πρακτικά - Συνεδρίαση ΔΣ03-2026.pdf"

    # Protocol row appended via OneDrive (not Google sheet)
    workflow._onedrive.append_protocol_row.assert_awaited_once()
    append_kwargs = workflow._onedrive.append_protocol_row.call_args.kwargs
    assert append_kwargs["protocol_id"] == "2026_006"
    assert append_kwargs["title"] == "Πρακτικά - Συνεδρίαση ΔΣ03-2026"
    assert append_kwargs["tags"] == "Διοικητικά, Πρακτικά"
    workflow._google.write_sheet.assert_not_called()
